...
def _record(day: int, total_cost: float, namespace_name: str = "payments"):
    return {
...
def _records(costs, namespace_name="payments"):
...
def test_rejects_non_positive_change_factor():
    with pytest.raises(ValueError, match="change_factor must be > 0"):
        detect_anomalies([_record(1, 10.0)], change_factor=0)


def test_detects_spike_above_moving_average_threshold():
    records = [
        _record(1, 10.0),
        _record(2, 10.0),
        _record(3, 10.0),
    ...
    assert anomalies[0]["threshold_value"] == 15.0
    assert anomalies[0]["severity"] == "LOW"


def test_anomaly_output_includes_severity_classification():
...
def test_value_equal_to_threshold_is_not_anomaly():
    records = [
        _record(1, 10.0),
        _record(2, 10.0),
        _record(3, 10.0),
...
def test_uses_configurable_parameters():
    records = [
        _record(1, 20.0),
        _record(2, 20.0),
        _record(3, 20.0),
    ...
    assert anomalies[0]["threshold_value"] == 24.0


def test_anomaly_output_includes_change_fields():
...
def test_payments_spike_is_marked_as_fast_change():
...
def test_checkout_jump_is_marked_as_fast_change():
...
def test_monitoring_gradual_increase_can_be_anomaly_without_fast_change():
...
def test_large_jump_after_small_recent_variation_is_fast_change():
...
def test_short_history_for_change_calculation_behaves_sensibly():
